...
use crate::{
    ...
    primitives::{
        ...
        types::{AccountId, BlockHeight, BlockId, BlockReference, GasPriceView, MaybeBlockId},
    },
};
...
impl NearRpcClient {
    ...
    pub async fn query(&self, request: RpcQueryRequest) -> Result<RpcQueryResponse, RpcError> {
        let params = serde_json::to_value(request).map_err(|e| RpcError {
            code: -1,
            message: e.to_string(),
        })?;
        let response = self.call_method("query", Some(params)).await?;
        let result = serde_json::from_value(response.result).map_err(|e| RpcError {
            code: -1,
            message: e.to_string(),
        })?;
